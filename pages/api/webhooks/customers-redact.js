...
async function validateShopifyWebhook(req, rawBodyString, res) {
    ...
    try {
        ...
        console.error('Shopify webhook validation error:', error.message, { category: 'webhook-customers-redact' });
        if (!res.headersSent) {
...
const handler = async (req, res) => {
    ...
    try {
        ...
        console.error('Failed to buffer request body:', bufError, { category: 'webhook-customers-redact' });
    ...
    console.info(`Received webhook for ${shop} with idempotency key ${idempotencyKey}`, { category: 'webhook-customers-redact' });
